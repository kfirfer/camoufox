...
### [X] Task 3.3: Build and package

- [X] **Step 1: Build**: `scripts/build-macos-native.sh build 2>&1 | tee /tmp/ff152-build.log` (plain `make build` fails on this host's SDK 27, see Global Constraints). That takes about 40 min cold and about 5 min incremental with ccache. Expected tail: `Your build was successful!` **Result:** `make build exit=0` in ~10 min (bootstrapped clang 20.1.8, SDK 26.5, rustc 1.95). mach suppresses the banner when it detects an agent; `--version` → `Camoufox Camoufox 152.0.4-beta.30`, `Version=152.0.4-beta.30` ✔
- [X] **Step 2: Smoke-run the binary**
  ```bash
...
- [X] **Step 3: Make pythonlib able to use the bundle** (§1.3-5). This mirrors upstream `service-tester/run_tests.sh`:
  ```bash
...
- [X] **Step 4 (optional, for distribution): package**: `make package-macos arch=arm64`. — not run (optional; dev build is used in place).
...
### [X] Task 4.1: `version.py`: detect the Firefox major from the binary
...
### [X] Task 4.3: `mcp_config.py` + thin CLI + `--mcp-package`
...
- [X] **Step 5: Golden check that the generated config is unchanged apart from the version**
  ```bash
...
### [X] Task 5.2: MCP end-to-end (F7, F8)

- [X] **Step 1: Re-register the MCP server with the 152 binary** (update `MISC.md` accordingly)
  ```bash
  ...
  **Result (2026-09-22):** closed the live 146 browser through the MCP (no `lock` left), backed up to `~/playwright-profile/profile-claude-camoufox.bak-146` (`diff -rq` clean, 69M), re-registered with the same flags; only `--executable-path` changed. `claude mcp get playwright` → Connected. `MISC.md` already points at 152. The profile is migrated on the first browser launch after the Claude Code session reconnects the MCP (`/mcp` → reconnect). **Rollback:** re-add with the 146 path and `rm -rf` the profile, then `mv profile-claude-camoufox.bak-146 profile-claude-camoufox`.
...
- [ ] **Step 4:** Remove the old tree **only after sign-off**: `rm -rf camoufox-146.0.1-beta.25 firefox-146.0.1.source.tar.xz` (both are gitignored, about 30 GB).
  **Blocker (found 2026-09-22):** four other projects' `playwright` MCPs (local scope in `~/.claude.json`) still use `camoufox-146.0.1-beta.25/.../camoufox`: `claude-1` (own launcher, `profile-claude-11`), `claude-2` (`profile-claude-2`), `claude-3` and `claude-4` (both via `claude-2/launch-camoufox-mcp.py`, `profile-claude-3`/`-4`). Two were running at the time. Deleting the tree breaks all four. Each needs its own re-registration + profile backup (same procedure as Task 5.2 Step 1) before this step. Their launcher copies are separate from `launch-camoufox-mcp.py` here (they run `@playwright/mcp@0.0.74 --timeout-action`), so check they work with 152 first.
  **Update (2026-09-22, later):** `claude-1` and `claude-2` were moved to 152 by the user (profiles `profile-claude-11`/`-2` now `LastVersion=152.0.4-beta.30`; neither has a `.bak-146`). Their launchers now pin `@playwright/mcp@0.0.82`, which bundles **Playwright 1.64.0-alpha**, i.e. above beta.30's `<1.63` pythonlib ceiling. Verified here with `launch-camoufox-mcp.py --mcp-package @playwright/mcp@0.0.82` on the 152 binary (scratch profile, fingerprint file restored): UA `rv:152.0`/`Firefox/152.0` in window = Worker = ServiceWorker and in the HTTP header; `--timezone Asia/Singapore` in all three; `Accept-Encoding: gzip, deflate, br, zstd`; cookie + localStorage survived a restart; screenshot OK. So 0.0.82 works in practice, although it is outside the tested floor/ceiling. Note 0.0.82 no longer returns page snapshots inline (written to `.playwright-mcp/`). **Still on 146:** `claude-3` and `claude-4` (`profile-claude-3`/`-4` still 146). Step 4 stays blocked until those two move.
